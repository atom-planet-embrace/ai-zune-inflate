...
fuzz_target!(|data: &[u8]| {
    if data.len() > 4
    {
        ...
        let options = ai_zune_inflate::DeflateOptions::default().set_limit(orig_len);
        let mut decoder = ai_zune_inflate::DeflateDecoder::new_with_options(&compressed, options);
        let decoded = decoder
            .decode_zlib()
